Tiempo promedio formacion de patron:
- frame-time (1hora):
- caso 1 (1 dia, 11 horas)
- caso 2 (1 dia, 15 horas) montaje sobre ema 20 / (1 dia, 23 horas ) salida
- caso 3 (1 dia, 17 horas)
- caso 4 (1 dia, 10 horas) salida

frame-time (4horas):
-caso 1 (4 dias, 2 hora)


Rsi - datos importantes que valos el rsi
- divergencia (agrgar filtro en para que si no esta sobre vendido en la zona del 26 para abajo no valore el patron)-pero agragar condional de la zona de gatillo zona 40-60.
- zona gatillo: valores 40-60 para que se active la entrada una vez que esta en por encima de la ema 20).
- Los extremos del rsi muchas veces pueden indicar la inciacion de un nuevo patron.



Volumen
 -


Dato importante
 - VERIFICAR SIEMPRE EN TIME-FRAME DE 4HS Y DIARIO QUE LAS VELAS DIBUJEN UN PATRON DE REVERSION (TRES VELAS COSECUTIVAS+DOJI FINAL) BUSCAR OTROS PATRONES DE VELAS JAPONESAS DE
   REVERSION.


 
